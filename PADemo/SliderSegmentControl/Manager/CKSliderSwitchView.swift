...
extension CKSlideSwitchView {
    ...
    fileprivate func createTabItemButtons() {
        ...
        for index in 0..<tabItemCount {
            let tabItem: UIButton = PAOffsetedSubviewButton(imageHeightRatio: 0.65, imageWidthRatio: 0, imageRounded: false, style: PAOffsetedSubviewButton.imageTopTitleBottom)
    ...
    fileprivate func configRootScrollView(_ runType: CKSlideSwitchViewRunType) {
        ...
        var counter: Int = 0
        print("rootContentViews.count = \(rootContentViews.count)")
...
//        if tabItemCount == 3 || tabItemCount == 4 {
        if tabItemCount == 3 {
            ...
            // 总页数大于小于3时
            rootScrollView.contentSize = CGSize(width: CGFloat(rootContentViews.count) * rootScrollView.frame.width, height: rootScrollView.frame.height)
...
//                }
    ...
    // 更新rootContentViews中的view，从而实现缓存效果
...
extension CKSlideSwitchView: UIScrollViewDelegate {
    func scrollViewDidScroll(_ scrollView: UIScrollView) {
        ...
        guard scrollView == rootScrollView, tabItemCount != 3 else { return }
        
        let rootScrollViewOffsetX = scrollView.contentOffset.x
        //print("rootScrollViewOffsetX == \(rootScrollViewOffsetX)")
        if rootScrollViewCurrentPage != 0 && rootScrollViewCurrentPage != tabItemCount - 1 {
